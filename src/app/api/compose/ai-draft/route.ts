...
export async function POST(request: Request) {
  try {
    ...
    // WordPress background path: create a pending post and queue Inngest job
    if (body.createDraft && account.platform !== "threads") {
      const pendingPost = await prisma.post.create({
        data: {
          ...
          textContent: "",
          excerpt: brief || "",
          topicTag: body.wordpressTemplate || "opinion",
          status: "generating",
      ...
      await inngest.send({
        name: "social-audio/ai.draft.requested",
        data: {
      ...
      return NextResponse.json({
        ...
        sourceLabel,
        resolvedUrl,
        message: "AI 正在背景生成草稿，約 30 秒後到草稿區查看"
      });
    }
