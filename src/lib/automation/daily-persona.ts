  ...
  for (let variantIndex = existingPosts.length; variantIndex < desiredCount; variantIndex += 1) {
    ...
    const post = await prisma.post.create({
      data: {
        ...
        excerpt: sourceMemory.rationale ? `${sourceMemory.laneLabel}｜${sourceMemory.rationale}` : null,
        status,
        scheduledAt,
    ...
    await prisma.automationLog.create({
      ...
      detail:
          status === "scheduled"
            ? `${detailPrefix}，並排入佇列（預計 ${scheduledAt?.toLocaleString("zh-TW", { hour12: false })} 發出，建議時段 ${timingSuggestion?.label ?? "即刻"}）。站台模式：${siteAutopilotMode}。來源層：${sourceMemory.laneLabel} / ${sourceMemory.confidence}。方向：${(account.autoGeneratePrompt || settings?.editorialDirection || "站台預設方向").slice(0, 36)}。Provider: ${result.provider}`
            : `${detailPrefix}，已存成候選草稿 ${variantIndex + 1} / ${desiredCount}。站台模式：${siteAutopilotMode}。來源層：${sourceMemory.laneLabel} / ${sourceMemory.confidence}。方向：${(account.autoGeneratePrompt || settings?.editorialDirection || "站台預設方向").slice(0, 36)}。Provider: ${result.provider}`
      }
    });
  }
